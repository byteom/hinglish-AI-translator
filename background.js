...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  } else if (info.menuItemId === "translateToTelugu" && info.selectionText) {
    doit(info, tab, "telugu");
  } else if (info.menuItemId === "translateToBengali" && info.selectionText) {
    doit(info, tab, "bengali");
  } else if (info.menuItemId === "translateToMarathi" && info.selectionText) {
    doit(info, tab, "marathi");
  } else if (info.menuItemId === "translateToTamil" && info.selectionText) {
    doit(info, tab, "tamil");
  } else if (info.menuItemId === "translateToKannada" && info.selectionText) {
    doit(info, tab, "kannada");
  }
});

async function doit(info, tab, language) {
  try {
    // Show loading popup
    chrome.scripting.executeScript({
      target: { tabId: tab.id },
    ...
    const translatedText = await translateTexttoLanguage(
      info.selectionText,
      language
    );
...
async function translateTexttoLanguage(text, language) {
  ...
  const prompt = `You are a language translator that converts English text into ${language}, 
but write the translated output using English letters only (transliteration). 
Keep the meaning accurate and make it sound natural and conversational in ${language}. 
Only respond with the transliterated version of the translated text, no explanation, no extra text.`;
